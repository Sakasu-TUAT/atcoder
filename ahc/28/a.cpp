...
int main()
{
    ...
    cerr << "before";
    cerr << calc_all_cost(ans) << '\n';
    double start_time = double(clock() - start) / CLOCKS_PER_SEC;
    ...
    while (time_limit(start))
    {
        for (int i = 0; i < ans.size(); i++)
        {
            for (int j = 0; j < ans.size(); j++)
            {
                ...
                // double temp = start_temp + (end_temp - start_temp) * (double(clock() - start) / CLOCKS_PER_SEC) / start_time;
                // double prob = exp((next_cost - prev_cost) / temp);
                // if (next_cost > prev_cost && prob < (double)rand() / RAND_MAX)
