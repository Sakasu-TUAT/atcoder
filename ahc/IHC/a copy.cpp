...
ll evaluate(vector<int>out,int k){
    vector<int>last(26,0);
    ll evaluated_score=0;
    rep(i,out.size()){
        last[out[i]]=i+1;
        rep(j,26)evaluated_score-=c[j]*(i+1-last[j]);
        evaluated_score+=s[i][out[i]];
    }
    for(ll i=out.size();i<min((int)out.size()+k,D);i++){
        rep(j,26)evaluated_score-=c[j]*(i+1-last[j]);
    }
...
void output(vector<int>out){
    rep(i,out.size())cout<<out[i]+1<<endl;
}
